...
def L_model_forward(X, parameters, use_batchnorm):
    ...
    '''
    caches = []
    net_size = int(len(parameters) / 2)  # since the dic has w and b for each layer
    A_current = X
    ...
    A_current, cache = linear_activation_forward(A_current, W, b, 'softmax')
    if use_batchnorm:
        A_current = apply_batchnorm(A_current)
...
def apply_batchnorm(A):
    ...
    nominator = A - mean_A
    epsilon = 0.001
    denominator = np.sqrt(var_A + epsilon)

    N_A = nominator / denominator
    return N_A
...
def compute_cost(AL, Y):
    ...
    AL_log = np.log(AL)
    AL_log_mult_y = np.multiply(AL_log, Y)
    all_samples_loss_vector = np.sum(AL_log_mult_y, axis=0)  # summing on columns
    total_loss = np.sum(all_samples_loss_vector, axis=0)

    m = AL.shape[1]  # number of samples in this batch
    cost = total_loss * (-1 / m)
...
def L_model_backward(AL, Y, caches):
    ...
    dA_prev, dW, db = linear_activation_backward(Y, cache_softmax_last_layer, 'softmax')
...
def Linear_backward(dZ, cache):
    ...
    # m = numer of samples
    m = A_prev.shape[1]
    # in the forward linear part , we do: Z = np.dot(W.T, A) + b
    dW = np.dot(dZ, A_prev.T) / m

    # db = np.sum(dZ, axis=0) / m  # should get (1, m) vector
...
def split_train_val(X, Y, ratio=0.2, axis=0):
...
def split_to_batch(X_train, Y_train, batch_size):
    ...
    if batch_size >= num_of_samples:
        X_batches.append(X_train)
        Y_batches.append(Y_train)
    else:
...
def L_layer_model(X, Y, layers_dims, learning_rate, num_iterations, batch_size, use_batchnorm):
    ...
    stopping_criteria = 0.05  # when the accuracy diff between iteration is less then 5% we stop
    stopping_criteriat_patience = 3  # we check for no improvempt of 3 consecutive iterations
    last_three_iter_acc = []

    # start of epoch ,  1 epoch == 1 iteration
    epoch = num_iterations
    for epoch_num in range(1, epoch + 1):
        print("# epoch number {}".format((str(epoch_num))))

        # each epoch we choose a different set og train and val
        X_train, Y_train, X_val, Y_val = split_train_val(X, Y, axis=1)
        ...
        for X_batch, Y_batch in zip(X_batches, Y_batches):
            ...
            # print("# iteration number {}".format((str(iter_num))))

            # training
            A_L, caches = L_model_forward(X_batch, parameters=w_b_parmas_dic, use_batchnorm=use_batchnorm)
            iter_cost = compute_cost(A_L, Y_batch)
            grads = L_model_backward(A_L, Y_batch, caches)
            w_b_parmas_dic = Update_parameters(w_b_parmas_dic, grads, learning_rate)

            train_accuracy = Predict(X_batch, Y_batch, w_b_parmas_dic)

            # validation
            val_accuracy = Predict(X_val, Y_val, w_b_parmas_dic)
            last_three_iter_acc.append(val_accuracy)
            if len(last_three_iter_acc) % stopping_criteriat_patience == 0:
                if last_three_iter_acc[2] - last_three_iter_acc[1] < stopping_criteria \
                        and last_three_iter_acc[1] - last_three_iter_acc[0] < stopping_criteria:
                    print("reached stopping criteria in iteration: {} , ACC : {}".format(str(iter_num),
                                                                                         str(val_accuracy)))
                    return w_b_parmas_dic, costs

            if iter_num % 100 == 0:
                print("# iter {} , train acc {} , val acc {}".format(str(iter_num), str(train_accuracy),
                                                                     str(val_accuracy)))
...
def Predict(X, Y, parameters):
    A_L, _ = L_model_forward(X, parameters, use_batchnorm=False)
