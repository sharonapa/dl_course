...
class SiameseNet:
    ...
    def train(self, train_dataset, y_array_train, val_dataset, y_array_val, save_model_path, batch_size=2, epoch=200):
        ...
        checkpoint_callback = ModelCheckpoint('model.{epoch:02d}-{val_loss:.4f}.h5',
    ...
    # def train_on_batch(self, train_dataset, y_array_train, val_dataset, y_array_val, batch_size=2, epoch=10):
    #     train_loss = []
    #     train_acc = []
    #     val_loss_arr = []
    #     val_acc_arr = []
    #
    #     # train data
    #     num_of_batches = len(train_dataset[0]) // batch_size
    #     first_image_batches = np.array_split(train_dataset[0], num_of_batches)
    #     second_image_batches = np.array_split(train_dataset[1], num_of_batches)
    #     y_batchs = np.array_split(y_array_train, num_of_batches)
    #
    #     # val data
    #     num_of_batches_val = len(val_dataset[0]) // batch_size
    #     first_image_batches_val = np.array_split(val_dataset[0], num_of_batches_val)
    #     second_image_batches_val = np.array_split(val_dataset[1], num_of_batches_val)
    #     y_batchs = np.array_split(y_array_val, num_of_batches_val)
    #
    ...
    #             x_batch = [first_image_batches[batch_index], second_image_batches[batch_index]]
    #             y_batch = y_batchs[batch_index]
    #             history = self.siam_model.train_on_batch(x_batch, y_batch)
    #             batches_loss += history[0]
    #             batches_acc += history[1]
    #         train_loss.append(batches_loss)
    #         batches_acc = batches_acc / float(len(train_dataset[0]))
    #         train_acc.append(batches_acc)
    #
    #         print('doing validation')
    #         val_acc = 0.0
    #         val_loss_batch = 0.0
    #         for batch_val_index in range(num_of_batches_val):
    #             x_batch_val = [first_image_batches_val[batch_val_index], second_image_batches_val[batch_val_index]]
    #             y_batch_val = y_batchs[batch_val_index]
    #
    #             y_pred_val = self.siam_model.predict_on_batch(x_batch_val)
    #             val_loss_batch += np.sum(np.abs(y_batch_val - y_pred_val.flatten()))
    #             val_acc += np.sum(y_batch_val == y_pred_val.flatten())
    #         val_acc = val_acc / float(len(val_dataset[0]))
    #         val_loss_arr.append(val_loss_batch)
    #         val_acc_arr.append(val_acc)
    #
    #         # connect to tensor board !!!!!!!!!
    #
    #         print('epoch {} , train loss {} , train acc {} , val loss {} , val acc {}'
    #               .format(e, train_loss[-1], train_acc[-1], val_loss[-1], val_acc[-1]))
